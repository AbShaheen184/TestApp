package j$.time;

import j$.time.temporal.TemporalAccessor;
import j$.time.temporal.TemporalUnit;

/* JADX INFO: loaded from: classes2.dex */
public final /* synthetic */ class d implements j$.time.temporal.m {
    public final /* synthetic */ int a;

    public /* synthetic */ d(int i) {
        this.a = i;
    }

    @Override // j$.time.temporal.m
    public j$.time.temporal.l e(j$.time.temporal.l lVar) {
        j$.time.temporal.a aVar = j$.time.temporal.a.DAY_OF_MONTH;
        return lVar.c(lVar.l(aVar).d, aVar);
    }

    public Object k(TemporalAccessor temporalAccessor) {
        int i = this.a;
        d dVar = j$.time.temporal.q.a;
        Object objU = null;
        switch (i) {
            case 0:
                return Instant.I(temporalAccessor);
            case 1:
                return LocalDate.O(temporalAccessor);
            case 2:
                return LocalDateTime.I(temporalAccessor);
            case 3:
                int i2 = OffsetDateTime.c;
                if (temporalAccessor instanceof OffsetDateTime) {
                    return (OffsetDateTime) temporalAccessor;
                }
                try {
                    ZoneOffset zoneOffsetJ0 = ZoneOffset.j0(temporalAccessor);
                    LocalDate localDate = (LocalDate) temporalAccessor.b(j$.time.temporal.q.f);
                    i iVar = (i) temporalAccessor.b(j$.time.temporal.q.g);
                    objU = (localDate == null || iVar == null) ? OffsetDateTime.u(Instant.I(temporalAccessor), zoneOffsetJ0) : new OffsetDateTime(LocalDateTime.R(localDate, iVar), zoneOffsetJ0);
                    return objU;
                } catch (DateTimeException e) {
                    j$.nio.file.k.g("Unable to obtain OffsetDateTime from TemporalAccessor: ", temporalAccessor, temporalAccessor.getClass().getName(), e);
                    return objU;
                }
            case 4:
                if (temporalAccessor instanceof ZonedDateTime) {
                    return (ZonedDateTime) temporalAccessor;
                }
                try {
                    ZoneId zoneIdU = ZoneId.u(temporalAccessor);
                    j$.time.temporal.a aVar = j$.time.temporal.a.INSTANT_SECONDS;
                    objU = temporalAccessor.h(aVar) ? ZonedDateTime.u(temporalAccessor.i(aVar), temporalAccessor.g(j$.time.temporal.a.NANO_OF_SECOND), zoneIdU) : ZonedDateTime.I(LocalDateTime.R(LocalDate.O(temporalAccessor), i.O(temporalAccessor)), zoneIdU, null);
                    return objU;
                } catch (DateTimeException e2) {
                    j$.nio.file.k.g("Unable to obtain ZonedDateTime from TemporalAccessor: ", temporalAccessor, temporalAccessor.getClass().getName(), e2);
                    return objU;
                }
            case 5:
                ZoneId zoneId = (ZoneId) temporalAccessor.b(dVar);
                if (zoneId == null || (zoneId instanceof ZoneOffset)) {
                    return null;
                }
                return zoneId;
            case 6:
            default:
                j$.time.temporal.a aVar2 = j$.time.temporal.a.NANO_OF_DAY;
                if (temporalAccessor.h(aVar2)) {
                    return i.g0(temporalAccessor.i(aVar2));
                }
                return null;
            case 7:
                return (ZoneId) temporalAccessor.b(dVar);
            case 8:
                return (j$.time.chrono.l) temporalAccessor.b(j$.time.temporal.q.b);
            case 9:
                return (TemporalUnit) temporalAccessor.b(j$.time.temporal.q.c);
            case 10:
                j$.time.temporal.a aVar3 = j$.time.temporal.a.OFFSET_SECONDS;
                if (temporalAccessor.h(aVar3)) {
                    return ZoneOffset.m0(temporalAccessor.g(aVar3));
                }
                return null;
            case 11:
                ZoneId zoneId2 = (ZoneId) temporalAccessor.b(dVar);
                return zoneId2 != null ? zoneId2 : (ZoneId) temporalAccessor.b(j$.time.temporal.q.d);
            case 12:
                j$.time.temporal.a aVar4 = j$.time.temporal.a.EPOCH_DAY;
                if (temporalAccessor.h(aVar4)) {
                    return LocalDate.o0(temporalAccessor.i(aVar4));
                }
                return null;
        }
    }

    public String toString() {
        switch (this.a) {
            case 7:
                return "ZoneId";
            case 8:
                return "Chronology";
            case 9:
                return "Precision";
            case 10:
                return "ZoneOffset";
            case 11:
                return "Zone";
            case 12:
                return "LocalDate";
            case 13:
                return "LocalTime";
            default:
                return super.toString();
        }
    }
}
